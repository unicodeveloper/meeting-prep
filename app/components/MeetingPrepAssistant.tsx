...
// Check app mode - in self-hosted mode, no auth is needed
...
export default function MeetingPrepAssistant() {
  ...
  useEffect(() => {
    // Only check for auth if in valyu mode
    if (!isValyuMode) return;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Only require sign-in if in valyu mode
    if (isValyuMode && !isSignedIn) {
  ...
  const generateBrief = async () => {
    ...
    try {
      ...
      // Only add auth header in valyu mode
      if (isValyuMode) {
        const accessToken = localStorage.getItem("valyuAccessToken");
        if (accessToken) {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || "Failed to generate meeting brief");
      }
